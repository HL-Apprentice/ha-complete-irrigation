...
<img width="1128" height="1064" alt="Yard tab: aerial with two tagged loop areas (Back Grass, Front Grass) and the read-out showing which loop drives the selected area, its size in sq ft, and the schedules that water it" src="https://raw.githubusercontent.com/HL-Apprentice/ha-complete-irrigation/main/assets/screenshots/panel-8.png" />
...
- **Plant records + yard map** — per-plant WUCOLS water-need math, need-vs-delivered per loop, photo history with EXIF-GPS placement, and an aerial yard map with draggable markers. **Zoomable** (20–120 m), **rotatable** (↺↻, 1° per tap — straighten a lot that sits at an angle to north, with a compass that turns with it), and **ground-anchored (v1.59)**: every marker stores a real lat/lon when you place it, so moving, zooming, nudging or rotating the aerial never loses a placement — a plant outside the current view just isn't drawn until the view covers it again.
- **Loop areas (v1.60)** — draw a box round the grass (or any bed) and tag the loop that waters it. Tap it any time to see which loop drives that ground, its area in sq ft, and every schedule that waters it. Regions are stored as real ground coordinates, so moving, zooming, nudging or rotating the aerial never moves or loses them.
