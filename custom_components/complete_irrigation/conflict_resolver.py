...
DEFAULT_CASCADE_CAP_MINUTES = 120  # match DEFAULT_CASCADE_CAP_HOURS=2
_MAX_ITERATIONS = 50
    ...
    while iterations < _MAX_ITERATIONS:
        ...
        for i in range(len(items) - 1):
            ...
            if policy == POLICY_DEFER_NEW:
                new_second_start = min_second_start
                if (new_second_start - second_orig) > cap:
                    items[i + 1] = (_mark_skip(second, "cascade-cap"), second_orig)
                else:
                    items[i + 1] = (
                        replace(second, start_at=new_second_start, reason="deferred"),
                        second_orig,
                    )
                adjusted = True
                ...
                if (first_orig - new_first_start) > cap:
                    # Fall back to defer policy for this pair
                    new_second_start = min_second_start
                    if (new_second_start - second_orig) > cap:
                        items[i + 1] = (_mark_skip(second, "cascade-cap"), second_orig)
                    else:
