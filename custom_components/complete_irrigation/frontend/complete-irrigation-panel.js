...
(function () {
  ...
  function emptyEditor() {
    return {
      ...
      // v1.17.13 — per-schedule weather-gate opt-outs
      ignore_wind: false,
  ...
  class CompleteIrrigationPanel extends HTMLElement {
    constructor() {
      ...
      // v1.17.13 — minute-tick so the day calendar's "now" line drifts
    ...
    connectedCallback() {
      ...
      this._scheduleRender();
      // v1.17.13 — Today is the initial section, so kick off the
    ...
    _startNowLineTimer() {
      // v1.17.13 — re-render every minute so the day-cal-now line drifts
    ...
    _onClick(e) {
      ...
      // v1.17.13 — info-bubble popover toggle.
    ...
    _onInput(e) {
      ...
      const t = e.target;
      if (!t) return;
      // v1.17.13 — live filter the sensor checkbox list as the user
    ...
    _navigateTo(sectionId) {
      ...
      // v1.17.13 — keep the now-line drifting only while Today is open.
    ...
    _openCopyOfSchedule(scheduleId) {
      // v1.17.13 — clone an existing schedule into the editor with a
    ...
    async _runSchedule(scheduleId, scheduleName) {
      // v1.17.13 — "Run" button on each schedule row. Confirms before
    ...
    _renderNotifications() {
      ...
      const notifyOnAborted = n.notify_on_aborted !== false; // default true (v1.17.13)
      // v1.17.13 — render the info bubble with a custom popover instead
    ...
    _renderSettings() {
      ...
      return (
        ...
        `</div>` +
        `</section>` +
        // v1.17.13 — admin-only services. Opt-in security hardening
    ...
    async _saveAdminOnlyServices(form) {
      // v1.17.13 — opt-in gate on hardware/CRUD service calls. Default
    ...
    _renderDayColumn(selected, offset) {
      ...
      // v1.17.13 — more visible now-line: 3px red line + a labeled chip
    ...
    _renderSensorCheckRows(entityIds, selected, inputName) {
      // v1.17.13 — extracted helper so the moisture + climate paths share
    ...
    _renderSensorPickerWithSearch(rowsHtml, kindKey, emptyMessage) {
      // v1.17.13 — wrap a sensor checklist with a search input. The
    ...
    _renderScheduleRow(s) {
      ...
      return (
        ...
        // v1.17.13 — "Run" button executes the schedule's full run
    ...
    _renderScheduleModal() {
      ...
      return (
        ...
        // v1.17.13 — per-schedule weather-gate opt-outs. Useful for
    ...
    _styles() {
      return (
        ...
        // v1.17.13 — Run-schedule button on each schedule row. Green
        ...
        // v1.17.13 — modal cap at 90vh + internal scroll, with the
        ...
        // v1.17.13 — sticky footer pinned to the modal's visible
        ...
        // v1.17.13 — enhanced "now" line with a left-edge labeled chip
        ...
        // v1.17.13 — info-bubble + custom popover. Native `title` was
        ...
        // v1.17.13 — search input above each sensor checklist. Live filters
        ...
        // mobile padding.
        // v1.17.13 — mobile modal: same scroll/sticky pattern as
