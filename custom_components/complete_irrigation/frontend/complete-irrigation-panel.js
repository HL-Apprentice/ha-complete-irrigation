 ...
 *
 * Vanilla Web Component (no Lit yet; we'll move to Lit when complexity
 * warrants it). Variant 2 layout per ADR-0002: sidebar nav + main content
 * with a collapsible sidebar. Today view shows the user's configured
 * zones from `hass.states`. Other sidebar sections are placeholders;
 * each lands in its own vertical slice (#3, #4, #6, #7, etc.).
 *
...
class CompleteIrrigationPanel extends HTMLElement {
  ...
  // Debounce re-renders since HA pushes hass updates frequently.
  _scheduleRender() {
  ...
  _render() {
    ...
    const navItems = SECTIONS.map(
      (s) => `
        <button class="sidebar-item ${s.id === this._currentSection ? "active" : ""}"
                data-section="${s.id}"
                title="${s.label}">
    ...
    // Wire up events
    this.shadowRoot.querySelectorAll(".sidebar-item").forEach((btn) => {
      btn.addEventListener("click", () => this._navigateTo(btn.dataset.section));
    });
    this.shadowRoot
      ...
      default: {
        ...
        return `
          <div class="placeholder">
            <h2>${section?.icon || ""} ${section?.label || "Section"}</h2>
  ...
  _renderToday() {
    ...
    return `
      ...
      <section>
        <h3 class="section-title">Zones (${zones.length})</h3>
        ${zones.length === 0 ? this._renderEmpty() : `
          <div class="zone-grid">
            ${zones.map((z) => this._renderZoneTile(z)).join("")}
          </div>
        `}
      </section>
  ...
  _renderEmpty() {
    return `
      <div class="empty">
        <p>No zones configured. Re-run setup from Settings → Devices & services.</p>
      </div>
    `;
  }
  ...
  // ── Styles (HA theme via CSS variables, with safe fallbacks) ─────
  _styles() {
    return `
      :host {
        display: block;
      ...
      * { box-sizing: border-box; }
      ...
      .sidebar-item.active {
        ...
        background: var(--primary-background-color, rgba(3,169,244,0.08));
...
customElements.define("complete-irrigation-panel", CompleteIrrigationPanel);
